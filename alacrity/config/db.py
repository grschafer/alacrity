...
def _get_mongo_connection():
    ...
    errorgame_collection = _config.get('db', 'errorgame_collection')
    userupload_collection = _config.get('db', 'userupload_collection')
    ...
    db[errorgame_collection].ensure_index('match_id', pymongo.ASCENDING)
    db[userupload_collection].ensure_index('match_id', pymongo.ASCENDING)
    return db

_db = _get_mongo_connection()
db = _db[_config.get('db', 'match_collection')]
league_db = _db[_config.get('db', 'league_collection')]
errorgame_db = _db[_config.get('db', 'errorgame_collection')]
userupload_db = _db[_config.get('db', 'userupload_collection')]
